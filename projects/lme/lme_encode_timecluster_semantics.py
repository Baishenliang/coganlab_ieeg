...
for alignment,xlim_align in zip(
    ...
    for elec_grp,elec_col,vWM_lambda,novWM_lambda,fea_plot_yscale in zip(('Sensorymotor','Auditory','Motor','Delay_only','Wgw_p55b','Wgw_a55b','SM_vWM_Auditory_early','SM_vWM_Auditory_late','SM_vWM_Delay','SM_vWM_Motor'),
        ...
        for fea, fea_tag, para_sig_barbar in zip(('ACC',),
            ...
            filename = f"results/LexDelayRep_{elec_grp}_{alignment}_Word_rawpow_vWMλ_{vWM_lambda}_novWMλ_{novWM_lambda}.csv"
            try:
            ...
            plt.tight_layout()
            plt.savefig(os.path.join('figs','semantics', f'{elec_grp}_{fea_tag}_{alignment}_Word_rawpow_vWMλ_{vWM_lambda}_novWMλ_{novWM_lambda}.tif'), dpi=100)
            plt.close()
            ...
            for is_vWM in ('_vWM','_novWM'):#','wordnessWord:aco','wordnessWord:pho'):
                ...
                for beta_fea in ('sem',):#('aco','pho','sem'):#,'wordnessWord:pho'):#,'wordnessWord:aco'
                    ...
                    plt.tight_layout()
                    plt.savefig(os.path.join('figs', 'semantics', f'{elec_grp}_{alignment}_{is_vWM}_{beta_fea.replace(":", "_")}_betas.tif'), dpi=100)
                    plt.close(fig)
                ...
                j=0
                # CHANGED: Loop variable updated to 'beta_fea' here as well
